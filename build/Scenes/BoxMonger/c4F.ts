module BOXMONGER {
    export class EqData {
        ...
        //X arrays of 2 numbers. First is for bloc type, second to know if it is already displayed or not
        private _worldData: any[];
        private _edgeSize: number;

        public static STONE: number = 1;
        public static GRASS: number = 2;
        public static SNOW: number = 3;
        ...
        public getData(x: number, y: number): any {
            return this._worldData[x * this._edgeSize + y];
        }

        public setData(x: number, y: number, value: number, blocType: number) {
            this._worldData[x * this._edgeSize + y] = { 'blockType': blocType, 'value': value, 'pitch': 0 };
        }

        public setPitch(x: number, y: number, pitch: number) {
            this._worldData[x * this._edgeSize + y].pitch = pitch;
        }

        public setBoxData(point: BABYLON.Vector2, dimensions: BABYLON.Vector2, value: number, blockType: number) {
    ...
    export class C4fWorldManager {
        private _worldData: EqData;
        private _edgeSize: number;
        private _grassBox: BoxMesh;
        private _grassHeight: number;
        private _snowBox: BoxMesh;
        private _snowHeight: number;
        private _stoneBox: BoxMesh;
        private _stoneHeight: number;
        private _analyzer: BABYLON.Analyser;
        private _scene: BABYLON.Scene;
        private _sounds: BABYLON.Sound[];
        private _soundVolume: number = 0.8;
        private _step: number = 1;
        private _pitchMultiplier: number = 10;
        private _updateEveryTweets: number = 50;
        private _freqStart: number = 0;
        private _freqStartUp: boolean = true;
        private _shootSound: BABYLON.Sound;
        private _geekCouncilSound: BABYLON.Sound;
        private _swoochSound: BABYLON.Sound;
        private _scratchSound: BABYLON.Sound;
        private _logoCoordinates: BABYLON.Vector2[];
        public tweetCount: number = 0;
        private _initialAudioStartTime: number;
        ...
        public startSounds() {
            ...
            var soundLoaded = () => {
                ...
                this._initialAudioStartTime = BABYLON.Engine.audioEngine.audioContext.currentTime;
                ...
                if (xhr.status === 200 || BABYLON.Tools.ValidateXHRData(xhr, 1)) {
                    try {
                        ...
                        for (var i = 0; i < nextTweets.length; i++) {
                            ...
                            this.displayTweet(nextTweets[i]);
                        }
                    }
                    catch (ex) {
        ...
        public static CreateTweetPlane(name: string, height: number, width: number, scene: BABYLON.Scene, updatable?: boolean): BABYLON.Mesh {
        ...
        public static CreateTweetPlaneVertexData(height: number, width: number): BABYLON.VertexData {
            var indices = [];
            var positions = [];
            var normals = [];
            var uvs = [];
        ...
        public moveSquare(up: boolean, distanceFromCenter: number) {
            var size = this._edgeSize / 5;
            var min = size;
            var max = this._edgeSize - size;
            var moved = false;
        ...
        public setHeightForBlockType(height: number, blockType: number, variance: number) {
            var size = this._edgeSize / 5;
