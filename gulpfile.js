...
var renderPage = function (pageConfig, globalConfig) {
  ...
  try {
    _fs.copySync(
      _path.join(pageConfig.absoluteRoot, "assets"),
      _path.join(dir, "assets")
    );
  } catch (ex) {
...
_gulp.task("build", function (done) {
  ...
  _handlebars.registerHelper({
    block: function (block) {
      var template = getTemplate(
        "./src/templates/" + block.templateName + "-template.html"
      );
    ...
    var: function (key) {
      ...
      else return new _handlebars.SafeString(key);
    },
    eq: function (v1, v2) {
      return v1 === v2;
    },
  ...
  siteConfig.menu.forEach(function (menuItem) {
    ...
    if (pageConfig.children) {
      for (var i = 0; i < pageConfig.children.length; i++) {
        pageConfig.children[i] = getPageConfig(
          _path.join(_contentRootPath, menuItem.root),
    ...
    if (pageConfig.visible) {
      ...
      if (pageConfig.absoluteRoot) globalConfig.menu.push(pageConfig);
    }
  });
  ...
  _fs.copySync(
    "./src/assets",
    _path.join(_path.resolve(), _outputRootPath, "assets")
  );
  ...
  siteConfig.menu.forEach(function (menuItem) {
    if (menuItem.menuUrl && menuItem.menuUrl.startsWith("http")) return;
